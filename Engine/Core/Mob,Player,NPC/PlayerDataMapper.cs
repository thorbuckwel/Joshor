...
namespace Engine
{
    ...
    {
        private static readonly string _connectionString = "Data Source=NAY;Initial Catalog=Joshor;Integrated Security=True";
        public static Player CreateFromDatabase(string pName)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    using (SqlCommand PlayerInfoCommand = connection.CreateCommand())
                    {
                        PlayerInfoCommand.CommandType = CommandType.Text;
                        ...
                        //PlayerInfoCommand.CommandText = "SELECT " + pName + " * FROM PlayerInfo";
                        PlayerInfoCommand.CommandText = "SELECT * FROM PlayerInfo WHERE Name LIKE '" + pName + "'";
                        // Use ExecuteReader when you expect the query to return a row, or rows
                        SqlDataReader reader = PlayerInfoCommand.ExecuteReader();
                        ...
                        string pclass = (string)reader["Class"];
                        string race = (string)reader["Race"];
                        Factions faction = (Factions)Enum.Parse(typeof(Factions), (string)reader["Faction"], true);
                        int gold = (int)reader["Gold"];
                        int exp = (int)reader["XP"];
                        int armor = (int)reader["AC"];
                        int currentHitPoints = (int)reader["CurrentHP"];
                        int maximumHitPoints = (int)reader["MaxHP"];
                        int equipt = (int)reader["Equipt"];
                        int align = (int)reader["Alignment"];
                        int currentLocationID = (int)reader["CurrentLocation"];

                        //// Create the Player object, with the saved game values
                        Player._player = new Player(name, pclass, race, gold, currentHitPoints, maximumHitPoints, World.WeaponByID(equipt), false, true, faction, align);
